...
window.setTimeout(function () {
  ...
  try {
    var recaptchaEl = document.querySelector('#form-field-pardot_recaptcha .elementor-g-recaptcha');
  ...
  var formElParent = document.querySelector('#post-grid li.post:nth-child(3)');

  if (formElParent.classList.contains('newsletter-signup')) {
    var formEl = document.querySelector('#post-grid li.post:nth-child(3) .elementor-location-single');
    var formElHeight = formEl.offsetHeight;
    formElParent.style.height = formElHeight + 30 + 'px';
    console.log("\uD83D\uDD14 Updating newsletter parent height to ".concat(formElHeight, "px and redrawing..."));
...
var mobileBreakPoint = postFilterVars.mobileBreakPoint;
...
var PostFilters = /*#__PURE__*/function () {
  function PostFilters() {
    ...
    this.filters = [];
    this.postType = postFilterVars.post_type;
    this.category = postFilterVars.category;
    this.gridId = postFilterVars.gridId;
    var allPosts = postFilterVars.posts;

    if (mobileBreakPoint >= browserWidth && 'post' == postFilterVars.post_type) {
      console.log("\uD83D\uDD14 Removing Newsletter Sign Up form because browser width is <= ".concat(mobileBreakPoint));
      allPosts.splice(2, 1);
    }

    this.posts = allPosts;
    this.defaultThumbnail = postFilterVars.defaultThumbnail;
    this.filterClassName = postFilterVars.filter_class_name;
    this.page_number = 1;
    this.page_size = parseInt(postFilterVars.limit);
    this.total_pages = -1 === this.page_size ? 1 : Math.ceil(this.posts.length / this.page_size);
    /*
    if( -1 === this.page_size ){
      this.total_pages = 1
    } else {
      if( typeof this.posts.length != undefined )
        this.total_pages = Math.ceil( this.posts.length/this.page_size )
    }
    */

    this.filterGroups = document.getElementsByClassName(postFilterVars.filter_class_name);
    ...
    value: function _getPostElement(el) {
      ...
      switch (this.postType) {
        ...
        default:
          ...
          if ('news' == postFilterVars.category) {
          ...
          if ('news' == postFilterVars.category) meta = '';
          var title = "<h3 class=\"\">".concat((0, _utilities.truncate)(el.title, 80, true), "</h3>");
          var listItem = "<li class=\"list-item ".concat(postFilterVars.category, " ").concat(this.postType, "\" data-groups='").concat(JSON.stringify(el.groups), "'><a href=\"").concat(el.permalink, "\"><div class=\"list-content ").concat(this.postType, "\">").concat(overlay).concat(meta).concat(title, "</div></a></li>");

          if ('newsletter_form' == el.title) {
            listItem = "<li class=\"list-item ".concat(this.postType, " newsletter-signup\" data-groups=\"[]\"><div class=\"list-content ").concat(this.postType, "\">").concat(postFilterVars.newsletterForm, "</div></li>");
          }
